...
- Small health potions and small mana potions enter a target-selection mode on right-click. They can be used on the local character or on another player character after the server confirms adjacent range; if the target is out of range but reachable, the server autoruns the player into adjacency before consuming the potion. Restorative potions are still consumed even when the target is already at full health or mana.
- Monster and player hits show red floating damage numbers in the viewport. Restorative potion use shows floating healing values on the healed character in red for health and blue for mana, while the sidebar health and mana bars mirror the authoritative character state.
