...
For generated item icons, use `apps/web/public/assets/items/leather_helmet.png`, `apps/web/public/assets/items/leather_armor.png`, `apps/web/public/assets/items/leather_legs.png`, and `apps/web/public/assets/items/leather_boots.png` as the primary quality benchmark. `dagger.png`, `wooden_shield.png`, and `brown_backpack.png` may be used as secondary category references only after the leather-set density, silhouette strength, texture, and polish bar is already met. Reject flat, vector-like, geometric, sparse, blurry, or undersized item results before committing them. Do not generate item icons with procedural canvas code, scripted geometry, or similar code-first shortcuts. Use `python apps/web/scripts/validate_item_icons.py` only as a technical sanity check for canvas size and subject occupancy, never as a replacement for side-by-side visual review against the benchmark icons.
